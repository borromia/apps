...
- [x] 1. Scaffold `weak-threads/index.html` with canvas, Matter.js CDN script tag, and Matter.js availability check
...
- [x] 4. Implement `LevelManager`
  - [x] 4.1 Implement `loadLevel(index)`
  ...
  - [x] 4.2 Implement `hitTestThread(x, y, threads)`
  ...
  - [x] 4.5 Implement `cutThread(thread)`
  ...
  - [x] 4.7 Implement `computeStars(totalThreads, threadsRemaining)`
  ...
  - [x] 4.9 Implement win handler (`onGemCaught`)
  ...
  - [x] 4.11 Implement lose condition check (called each frame)
    - If `gem.position.y > canvas.height + 100` and `gameState === 'PLAYING'`, set `gameState = 'LOSE'`
    - _Requirements: 7.1_
  - [x] 4.12 Implement game-complete handling
...
- [x] 8. Implement the main game loop and wire all components together
  - [x] 8.1 Implement `gameLoop(timestamp)` using `requestAnimationFrame`
  ...
  - [x] 8.3 Wire `InputHandler.onCut` → `LevelManager.hitTestThread` → `LevelManager.cutThread` → `Renderer.spawnParticles`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.6_
  - [x] 8.4 Wire `PhysicsEngine.onCollisionStart` → `LevelManager.onGemCaught` → `Renderer` win overlay
    - _Requirements: 4.1, 4.2, 4.4_
  - [x] 8.5 Wire restart button and "Try Again" overlay click → `loadLevel(currentIndex)`
    - _Requirements: 7.3_
